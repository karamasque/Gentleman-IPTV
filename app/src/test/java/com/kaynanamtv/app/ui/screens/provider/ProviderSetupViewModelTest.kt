...
class ProviderSetupViewModelTest {
    ...
    @Test
    fun `login xtream success automatically sets new provider as active and default`() = runTest {
        val createdProvider = Provider(
            id = 7L,
            name = "New IPTV",
        ...
        assertThat(viewModel.uiState.value.onboardingCompletion)
            .isEqualTo(ProviderSetupViewModel.OnboardingCompletion.READY)
        assertThat(viewModel.uiState.value.createdProviderId).isEqualTo(7L)
        verify(preferencesRepository).setLastActiveProviderId(7L)
        verify(preferencesRepository).setActiveLiveSource(ActiveLiveSource.ProviderSource(7L))
    ...
    fun `stalker source defaults epg sync mode to background when user has not customized it`() = runTest {
        ...
        assertThat(viewModel.uiState.value.epgSyncMode).isEqualTo(ProviderEpgSyncMode.UPFRONT)
