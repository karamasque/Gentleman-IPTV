...
@Serializable
data class KaynanamTVPluginManifest(
    ...
    val supportsConfigurationActivity: Boolean
        get() = hasCapability(KaynanamTVPluginContract.CAPABILITY_CONFIGURATION_ACTIVITY) &&
            !configurationActivityAction.isNullOrBlank()

    val usesActivityConfiguration: Boolean
        get() = configurationMode == KaynanamTVPluginContract.CONFIGURATION_MODE_ACTIVITY

    val supportsHostRenderedConfiguration: Boolean
        get() = configurationMode == KaynanamTVPluginContract.CONFIGURATION_MODE_HOST_SCHEMA ||
            (configurationMode != KaynanamTVPluginContract.CONFIGURATION_MODE_ACTIVITY &&
                hasCapability(KaynanamTVPluginContract.CAPABILITY_CONFIGURATION_SCHEMA))

    val canConfigure: Boolean
        get() = supportsHostRenderedConfiguration || supportsConfigurationActivity
}

data class InstalledKaynanamTVPlugin(
    val packageName: String,
    val serviceClassName: String,
    val appLabel: String,
    val manifest: KaynanamTVPluginManifest,
    val enabled: Boolean,
...
data class PluginConfigurationSnapshot(
    val plugin: InstalledKaynanamTVPlugin,
