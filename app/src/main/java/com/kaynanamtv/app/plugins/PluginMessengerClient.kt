...
class PluginMessengerClient @Inject constructor(
    ...
    suspend fun send(
        ...
        val replyMessenger = Messenger(Handler(Looper.getMainLooper()) { message ->
            ...
            if (response.getString(KaynanamTVPluginContract.KEY_REQUEST_ID) == requestId &&
        ...
        try {
            withContext(Dispatchers.Main.immediate) {
                val intent = Intent(KaynanamTVPluginContract.ACTION_PLUGIN_SERVICE).apply {
            ...
            val request = Message.obtain(null, what).apply {
                ...
                this.data = Bundle(data).apply {
                    putInt(KaynanamTVPluginContract.KEY_API_VERSION, KaynanamTVPluginContract.API_VERSION)
                    putString(KaynanamTVPluginContract.KEY_REQUEST_ID, requestId)
                }
            }
            try {
