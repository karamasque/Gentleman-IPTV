package com.kaynanamtv.app.backup
...
object BackupFileBridge {
    ...
    fun createExportFile(baseDir: File): File {
        ...
        return File(backupsDir, "kaynanamtv_backup_${LocalDateTime.now().format(exportNameFormatter)}.json")
            .also { file ->
    ...
    fun copyToImportInbox(context: Context, sourceUri: Uri): Uri? {
        ...
        val targetFile = File(inboxDir, "kaynanamtv_import_${System.currentTimeMillis()}.json")
        return runCatching {
    ...
    fun buildShareIntent(uri: Uri): Intent {
        ...
        return Intent.createChooser(sendIntent, "Share KaynanamTV backup")
            .addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)
    }
