...
import com.kaynanamtv.app.di.MainPlayerEngine
import com.kaynanamtv.app.player.LivePreviewHandoffManager
import com.kaynanamtv.app.player.LiveTranslationSession
import com.kaynanamtv.app.plugins.KaynanamTVPluginManager
import com.kaynanamtv.app.util.isPlaybackComplete
...
import com.kaynanamtv.data.remote.stalker.StalkerUrlFactory
import com.kaynanamtv.data.remote.xtream.XtreamStreamUrlResolver
...
import com.kaynanamtv.domain.model.CombinedCategory
import com.kaynanamtv.domain.model.CombinedM3uProfileMember
...
import com.kaynanamtv.domain.usecase.MarkAsWatched
import com.kaynanamtv.domain.usecase.ScheduleRecording
...
import com.kaynanamtv.player.AUDIO_VIDEO_OFFSET_MAX_MS
import com.kaynanamtv.player.AUDIO_VIDEO_OFFSET_MIN_MS
...
import com.kaynanamtv.player.PlayerSubtitleStyle
...
import com.kaynanamtv.player.playback.applyUnsafeTlsBypass
...
class PlayerViewModel @Inject constructor(
    ...
    internal val mainPlayerEngine: PlayerEngine,
    ...
    internal val favoriteRepository: com.kaynanamtv.domain.repository.FavoriteRepository,
    internal val playbackHistoryRepository: PlaybackHistoryRepository,
    internal val providerRepository: com.kaynanamtv.domain.repository.ProviderRepository,
    internal val combinedM3uRepository: CombinedM3uRepository,
    internal val preferencesRepository: com.kaynanamtv.data.preferences.PreferencesRepository,
    ...
    internal val pluginManager: KaynanamTVPluginManager,
    ...
    internal val currentChannelFlow = MutableStateFlow<com.kaynanamtv.domain.model.Channel?>(null)
    val currentChannel: StateFlow<com.kaynanamtv.domain.model.Channel?> = currentChannelFlow.asStateFlow()
    ...
    internal val currentChannelFlowList = MutableStateFlow<List<com.kaynanamtv.domain.model.Channel>>(emptyList())
    val currentChannelList: StateFlow<List<com.kaynanamtv.domain.model.Channel>> = currentChannelFlowList.asStateFlow()

    internal val recentChannelsFlow = MutableStateFlow<List<com.kaynanamtv.domain.model.Channel>>(emptyList())
    val recentChannels: StateFlow<List<com.kaynanamtv.domain.model.Channel>> = recentChannelsFlow.asStateFlow()
    ...
    private var activePlaybackTrafficProviderId: Long = -1L
    ...
    val remoteShortcutPreferences = preferencesRepository.remoteShortcutPreferences
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000L), com.kaynanamtv.domain.model.RemoteShortcutPreferences())
    ...
    /** Aktif TeeCapture kaydının DB kimliği; null ise aktif kayıt yok. */
    internal var _activeTeeRecordingId: String? = null
    ...
    private var preferredSurfaceMode: com.kaynanamtv.domain.model.PlayerSurfaceMode =
        com.kaynanamtv.domain.model.PlayerSurfaceMode.AUTO
     ...
     */
    internal var channelList: List<com.kaynanamtv.domain.model.Channel> = emptyList()
        set(value) {
            field = value
            rebuildChannelNumberIndex()
        }
    internal var channelNumberIndex: Map<Int, com.kaynanamtv.domain.model.Channel> = emptyMap()
        private set
    ...
    internal fun setActivePlayerEngine(engine: PlayerEngine) {
    ...
    internal fun logRepositoryFailure(operation: String, result: com.kaynanamtv.domain.model.Result<Unit>) {
    ...
    val playerStats: StateFlow<com.kaynanamtv.player.PlayerStats> =
        activeEngineState(com.kaynanamtv.player.PlayerStats()) { it.playerStats }
    val availableAudioTracks: StateFlow<List<com.kaynanamtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableAudioTracks }
    }
    val availableSubtitleTracks: StateFlow<List<com.kaynanamtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
        activeEngineState(emptyList()) { it.availableSubtitleTracks }
    }
    val availableVideoQualities: StateFlow<List<com.kaynanamtv.player.PlayerTrack>> by lazy(LazyThreadSafetyMode.NONE) {
    ...
    private suspend fun probePlaybackUrl(streamInfo: com.kaynanamtv.domain.model.StreamInfo): PlaybackProbeFailure? {
    ...
    private suspend fun shouldProbePlaybackUrl(url: String): Boolean {
        ...
        if (provider.type != com.kaynanamtv.domain.model.ProviderType.STALKER_PORTAL && cacheKey in probePassedPlaybackKeys) {
            return false
        }
        return (
            provider.type == com.kaynanamtv.domain.model.ProviderType.XTREAM_CODES ||
                provider.type == com.kaynanamtv.domain.model.ProviderType.STALKER_PORTAL
            ) &&
    ...
    fun prepare(
        ...
        if (providerId > 0L) {
            if (activePlaybackTrafficProviderId > 0L && activePlaybackTrafficProviderId != providerId) {
                syncManager.notePlaybackStopped(activePlaybackTrafficProviderId)
            }
            activePlaybackTrafficProviderId = providerId
            syncManager.notePlaybackStarted(providerId)
        }
    ...
    internal fun updateChannelDiagnostics(channel: com.kaynanamtv.domain.model.Channel) {
    ...
    internal suspend fun resolvePlaybackStreamInfo(
        ...
        providerId: Long,
        contentType: ContentType
    ): com.kaynanamtv.domain.model.StreamInfo? {
    ...
    override fun onCleared() {
        super.onCleared()
        if (activePlaybackTrafficProviderId > 0L) {
            syncManager.notePlaybackStopped(activePlaybackTrafficProviderId)
            activePlaybackTrafficProviderId = -1L
        }
