...
import com.kaynanamtv.data.sync.SyncRepairSection
import com.kaynanamtv.domain.manager.BackupConflictStrategy
import com.kaynanamtv.domain.manager.BackupImportPlan
import com.kaynanamtv.domain.manager.BackupManager
import com.kaynanamtv.domain.manager.BackupPreview
import com.kaynanamtv.domain.manager.DriveBackupSyncManager
...
import com.kaynanamtv.domain.usecase.ExportBackup
import com.kaynanamtv.domain.usecase.ExportBackupCommand
import com.kaynanamtv.domain.usecase.ExportBackupResult
import com.kaynanamtv.domain.usecase.ImportBackup
import com.kaynanamtv.domain.usecase.ImportBackupCommand
import com.kaynanamtv.domain.usecase.ImportBackupResult
import com.kaynanamtv.domain.usecase.InspectBackupCommand
import com.kaynanamtv.domain.usecase.InspectBackupResult
...
import com.kaynanamtv.domain.usecase.SyncProviderCommand
import com.kaynanamtv.domain.usecase.SyncProviderResult
import com.kaynanamtv.player.AudioCompatibilityMemoryStore
...
private const val BACKGROUND_INDEX_STATUS_PREFIX = "Arka plan indeksi:"
...
class SettingsViewModel @Inject constructor(
    ...
    private val playbackHistoryRepository: com.kaynanamtv.domain.repository.PlaybackHistoryRepository,
    ...
    private val epgSourceRepository: com.kaynanamtv.domain.repository.EpgSourceRepository,
    ...
    private val databaseMaintenanceManager: com.kaynanamtv.data.sync.DatabaseMaintenanceManager
) : ViewModel() {
    ...
    val backgroundSyncIntervalHours: StateFlow<Int> =
        preferencesRepository.backgroundSyncIntervalHours.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5_000),
            initialValue = 6
        )
    val backgroundSyncWifiOnly: StateFlow<Boolean> =
        preferencesRepository.backgroundSyncWifiOnly.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5_000),
            initialValue = false
        )
    val lastBackgroundSyncTimestamp: StateFlow<Long?> =
        preferencesRepository.lastBackgroundSyncTimestamp.stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5_000),
            initialValue = null
        )

    private val _storageBreakdown = MutableStateFlow(com.kaynanamtv.data.sync.DatabaseMaintenanceManager.StorageBreakdown())
    val storageBreakdown: StateFlow<com.kaynanamtv.data.sync.DatabaseMaintenanceManager.StorageBreakdown> = _storageBreakdown.asStateFlow()
    ...
    private val _lastFreedStorageBytes = MutableStateFlow<Long?>(null)
    val lastFreedStorageBytes: StateFlow<Long?> = _lastFreedStorageBytes.asStateFlow()
    ...
    fun refreshStorageBreakdown() {
        viewModelScope.launch {
            _storageBreakdown.value = databaseMaintenanceManager.getAppStorageBreakdown(appContext)
        }
    }

    fun cleanAppStorageAndCache() {
        viewModelScope.launch {
            _isCleaningStorage.value = true
            try {
                val report = databaseMaintenanceManager.cleanAllStorageAndCache(appContext)
                _storageBreakdown.value = report.finalBreakdown
                _lastFreedStorageBytes.value = report.freedBytes
            } catch (e: Exception) {
                Log.e("SettingsViewModel", "Failed to clean storage", e)
            } finally {
    ...
    private fun XtreamIndexJobEntity.toSettingsWarningMessage(): String? {
        val label = when (section.uppercase()) {
            "LIVE" -> "Canlı TV"
            "MOVIE" -> "Film"
            "SERIES" -> "Dizi"
            "EPG" -> "Rehber (EPG)"
        ...
        return when (state.uppercase()) {
            "RUNNING" -> {
                val detail = if (totalCategories > 0) {
                    val pct = (completedCategories * 100) / totalCategories
                    " (%$pct • $completedCategories/$totalCategories kategori • ${indexedRows} kayıt)"
                } else if (indexedRows > 0) {
                    " (${indexedRows} kayıt indirildi)"
                } else {
                    " güncelleniyor..."
                }
                "$BACKGROUND_INDEX_STATUS_PREFIX $label kataloğu arka planda güncelleniyor$detail"
            }
            "QUEUED" -> "$BACKGROUND_INDEX_STATUS_PREFIX $label kataloğu sırada..."
            "PARTIAL" -> "$BACKGROUND_INDEX_STATUS_PREFIX $label kısmi: ${indexedRows} indekslendi"
            "FAILED_RETRYABLE" -> "$BACKGROUND_INDEX_STATUS_PREFIX $label yeniden denenebilir hatası var${lastError?.let { ": $it" }.orEmpty()}"
            "FAILED_PERMANENT" -> "$BACKGROUND_INDEX_STATUS_PREFIX $label başarısız oldu${lastError?.let { ": $it" }.orEmpty()}"
            else -> null
        }
    }
    ...
    fun setStartupSoundOption(option: Int) {
        viewModelScope.launch {
            preferencesRepository.setStartupSoundOption(option)
        }
    }
    ...
    fun setBackgroundSyncIntervalHours(hours: Int) {
        viewModelScope.launch {
            preferencesRepository.setBackgroundSyncIntervalHours(hours)
        }
    }

    fun setBackgroundSyncWifiOnly(wifiOnly: Boolean) {
        viewModelScope.launch {
            preferencesRepository.setBackgroundSyncWifiOnly(wifiOnly)
        }
    }
    ...
    fun setPlayerSurfaceMode(mode: com.kaynanamtv.domain.model.PlayerSurfaceMode) {
        viewModelScope.launch {
