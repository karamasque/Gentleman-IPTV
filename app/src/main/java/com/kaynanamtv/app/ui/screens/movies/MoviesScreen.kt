...
fun MoviesScreen(
    ...
    if (uiState.showDialog && uiState.selectedMovieForDialog != null) {
        val movie = uiState.selectedMovieForDialog ?: return
        ...
        if (uiState.isLoadingSelectedCategory) {
            ...
            item(span = { GridItemSpan(maxLineSpan) }) {
                Box(
                    ...
                    AppMessageState(
                        title = if (uiState.hasProviders && uiState.categories.isEmpty() && filteredGridMovies.isEmpty()) {
                            "İçerikler hazırlanıyor…"
                        } else {
                            stringResource(R.string.movies_no_found)
                        },
                        subtitle = if (uiState.hasProviders && uiState.categories.isEmpty() && filteredGridMovies.isEmpty()) {
                            "Filmler arka planda indiriliyor ve hazırlanıyor, lütfen birazdan tekrar kontrol edin."
                        } else {
                            stringResource(R.string.movies_no_found_subtitle)
                        }
                    )
                }
            }
...
private fun movieFilterChips(): List<SelectionChip> {
    return listOf(
        ...
        SelectionChip(LibraryFilterType.RECENTLY_UPDATED.name, "Son Eklenenler"),
