...
import com.kaynanamtv.data.local.entity.ProgramEntity
    ...
    override suspend fun refreshEpg(providerId: Long, epgUrl: String): Result<Unit> =
        withContext(Dispatchers.IO) {
            if (com.kaynanamtv.data.remote.stalker.StalkerTrafficCoordinator.shouldDeferCatalogFetch(providerId) ||
                ...
                Log.d("EpgRepository", "Deferring EPG XMLTV refresh for provider $providerId due to active playback")
                return@withContext Result.error("EPG XMLTV refresh deferred during active playback")
            }
            providerRefreshMutex(providerId).withLock {
                ...
                try {
                    ...
                    if (programsParsed > 0) {
                        Log.w("EpgRepository", "EPG parsing encountered error but committed $programsParsed successfully parsed programs: ${e.message}")
                        flushBatch()
                        transactionRunner.inTransaction {
                            programDao.deleteByProvider(providerId)
                            programDao.moveToProvider(stagingProviderId, providerId)
