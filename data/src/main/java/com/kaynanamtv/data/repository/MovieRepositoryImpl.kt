...
import com.kaynanamtv.data.local.entity.MovieBrowseEntity
...
import com.kaynanamtv.data.sync.ContentCachePolicy
import com.kaynanamtv.data.sync.SyncManager
import com.kaynanamtv.data.util.MoviePresentationSettings
import com.kaynanamtv.data.util.buildPresentedMovies
import com.kaynanamtv.data.util.rankSearchResults
import com.kaynanamtv.data.util.toFtsPrefixQuery
...
import com.kaynanamtv.domain.model.Result.Success
...
class MovieRepositoryImpl @Inject constructor(
    ...
    private suspend fun loadPresentationCandidates(movie: Movie): List<com.kaynanamtv.data.local.entity.MovieEntity> {
    ...
    private suspend fun ensureXtreamCategoryLoaded(
        ...
        if (provider.type == ProviderType.XTREAM_CODES) {
            if (localCount > 0) {
                ...
                triggerXtreamCategoryIndexing(providerId, ContentType.MOVIE)
            }
            return
        }
        if (provider.type == ProviderType.STALKER_PORTAL) {
    ...
    private fun triggerXtreamCategoryIndexing(providerId: Long, contentType: ContentType) {
        val key = "xtream_indexing:$providerId:${contentType.name}"
        if (!backgroundRefreshes.add(key)) return
        repositoryScope.launch {
            try {
                syncManager.processQueuedXtreamIndexJobs(providerId, contentType, force = false, maxCategoriesPerSection = 50)
            } catch (e: Exception) {
                Log.w(TAG, "Foreground category index trigger failed for provider $providerId ($contentType)", e)
            } finally {
    ...
    private fun com.kaynanamtv.data.local.entity.MovieEntity.hasFreshXtreamDetails(now: Long = System.currentTimeMillis()): Boolean {
