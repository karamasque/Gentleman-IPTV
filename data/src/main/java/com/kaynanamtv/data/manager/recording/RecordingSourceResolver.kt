...
import com.kaynanamtv.data.remote.xtream.ResolvedStreamUrl
...
class RecordingSourceResolver @Inject constructor(
    ...
    suspend fun resolveLiveSource(
        ...
        val resolved = xtreamStreamUrlResolver.resolveWithMetadata(
            ...
            // Use the credential-based portal URL for recording — the stable CDN
            // URL may be token-locked and return 403 when accessed without the
            // session context that the player established. The portal URL is
            // longer-lived and generally works for background recording sessions.
        ...
        val providerLabel = providerDao.getById(providerId)?.let { provider ->
            when {
                provider.type == com.kaynanamtv.domain.model.ProviderType.XTREAM_CODES -> "${provider.name} • Xtream"
                provider.type == com.kaynanamtv.domain.model.ProviderType.M3U -> "${provider.name} • M3U"
                provider.type == com.kaynanamtv.domain.model.ProviderType.STALKER_PORTAL -> "${provider.name} • Stalker"
                else -> provider.name
            }
        }
