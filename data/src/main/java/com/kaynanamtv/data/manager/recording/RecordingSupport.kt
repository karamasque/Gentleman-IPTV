package com.kaynanamtv.data.manager.recording
...
import com.kaynanamtv.data.local.entity.RecordingRunWithSchedule
...
internal fun sanitizeRecordingFileName(
    ...
    return when {
        rendered.endsWith(".ts", ignoreCase = true) -> rendered
        rendered.endsWith(".mp4", ignoreCase = true) -> rendered
        else -> "$rendered.ts"
    }
}
...
internal fun resolveStorageDetails(
    ...
    if (treeUriString.isNullOrBlank()) {
        val recordingsDir = localDirectory
            ...
            ?.takeIf { it.canWrite() || (it.mkdirs() && it.canWrite()) }
        ...
        // Internal app storage is always writable – never block recording because of
        // an unreachable external path.
        val writable = recordingsDir.canWrite() || recordingsDir.parentFile?.canWrite() == true
        return Triple(recordingsDir.absolutePath, available, writable)
    }
    ...
    // If the SAF tree is not writable (permission revoked, drive removed, etc.)
    // fall back to internal storage so recording is never silently blocked.
    if (!isWritable) {
        val fallback = defaultRecordingDirectory(context)
        val available = runCatching { StatFs(fallback.absolutePath).availableBytes }.getOrNull()
            ?.takeIf { it > 0L } ?: fallback.usableSpace.takeIf { it > 0L }
        return Triple(fallback.absolutePath, available, fallback.canWrite())
    }
    return Triple(documentTree?.name ?: treeUriString, null, true)
}
...
private fun defaultRecordingDirectory(context: Context): File {
    // 1. Prefer shared public Movies directory (/storage/emulated/0/Movies/KaynanamTV)
    // so recordings appear directly in Galeri, Google Photos, Files app, and PC/MTP transfers.
    val publicMoviesDir = runCatching {
        val moviesDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_MOVIES)
        val kaynanamTvDir = File(moviesDir, "KaynanamTV")
        if (!kaynanamTvDir.exists()) {
            kaynanamTvDir.mkdirs()
        }
        kaynanamTvDir.takeIf { it.canWrite() || (it.mkdirs() && it.canWrite()) }
    }.getOrNull()

    // 2. Fall back to external app-private directory if public dir is unavailable
    val externalPrivateDir = runCatching {
    ...
    // 3. Fall back to internal app storage
    val targetDir = publicMoviesDir
        ?: externalPrivateDir
        ?: File(context.filesDir, DEFAULT_RECORDINGS_DIR_NAME)
...
/**
 * Adds the completed recording file to the device MediaStore (Gallery / Files app).
 * On API 29+ uses MediaStore.Video.Media; also triggers MediaScannerConnection on all API levels.
 * Errors are swallowed so a failure here never breaks the recording completion flow.
 */
internal fun addRecordingToMediaStore(
    context: Context,
    filePath: String,
    channelName: String,
    programTitle: String?,
    ...
    runCatching {
        ...
        val title = listOfNotNull(channelName.takeIf { it.isNotBlank() }, programTitle?.takeIf { it.isNotBlank() })
            .joinToString(" – ").ifBlank { displayName }
        val mimeType = if (displayName.endsWith(".mp4", ignoreCase = true)) "video/mp4" else "video/mp2t"
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
            ...
            if (existing == null) {
                val contentValues = android.content.ContentValues().apply {
                    ...
                    put(android.provider.MediaStore.Video.Media.TITLE, title)
                    put(android.provider.MediaStore.Video.Media.MIME_TYPE, mimeType)
                    put(android.provider.MediaStore.Video.Media.RELATIVE_PATH, "Movies/KaynanamTV")
                    ...
                    durationMs?.let { put(android.provider.MediaStore.Video.Media.DURATION, it) }
                }
                context.contentResolver.insert(collection, contentValues)
                android.util.Log.i("RecordingMediaStore", "Added recording to MediaStore: $displayName")
            }
        }

        // Trigger MediaScannerConnection on all Android versions so Gallery apps immediately index the file
        android.media.MediaScannerConnection.scanFile(
            context,
            arrayOf(filePath),
            arrayOf(mimeType, "video/mp4", "video/*"),
            null
        )
        android.util.Log.i("RecordingMediaStore", "Triggered media scan for: $displayName")
    }.onFailure { error ->
        android.util.Log.w("RecordingMediaStore", "Failed to add recording to MediaStore", error)
    }
}
