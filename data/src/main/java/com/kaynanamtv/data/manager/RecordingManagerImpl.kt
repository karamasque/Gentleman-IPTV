...
import com.kaynanamtv.data.local.dao.RecordingScheduleDao
import com.kaynanamtv.data.local.dao.RecordingStorageDao
import com.kaynanamtv.data.local.entity.RecordingRunEntity
import com.kaynanamtv.data.local.entity.RecordingScheduleEntity
import com.kaynanamtv.data.local.entity.RecordingStorageEntity
import com.kaynanamtv.data.manager.recording.CaptureProgress
import com.kaynanamtv.data.manager.recording.HlsLiveCaptureEngine
import com.kaynanamtv.data.manager.recording.RecordingAlarmScheduler
import com.kaynanamtv.data.manager.recording.RecordingForegroundService
import com.kaynanamtv.data.manager.recording.RecordingOutputTarget
import com.kaynanamtv.data.manager.recording.RecordingServiceLauncher
import com.kaynanamtv.data.manager.recording.RecordingSourceResolver
import com.kaynanamtv.data.manager.recording.ResolvedRecordingSource
import com.kaynanamtv.data.manager.recording.TsPassThroughCaptureEngine
import com.kaynanamtv.data.manager.recording.UnsupportedRecordingException
import com.kaynanamtv.data.preferences.PreferencesRepository
import com.kaynanamtv.data.manager.recording.asPersistenceValues
import com.kaynanamtv.data.manager.recording.createOutputTarget
import com.kaynanamtv.data.manager.recording.deleteOutputTarget
import com.kaynanamtv.data.manager.recording.headersFromJson
import com.kaynanamtv.data.manager.recording.headersToJson
import com.kaynanamtv.data.manager.recording.inferFailureCategory
import com.kaynanamtv.data.manager.recording.resolveStorageDetails
import com.kaynanamtv.data.manager.recording.addRecordingToMediaStore
import com.kaynanamtv.data.manager.recording.sanitizeRecordingFileName
import com.kaynanamtv.data.manager.recording.toEntity
import com.kaynanamtv.data.manager.recording.toDomain
import com.kaynanamtv.domain.manager.RecordingManager
import com.kaynanamtv.domain.model.RecordingFailureCategory
...
import com.kaynanamtv.domain.model.RecordingSourceType
...
private const val FAILURE_NOTIFICATION_CHANNEL_ID = "kaynanamtv_recording_failure"
...
class RecordingManagerImpl @Inject constructor(
    ...
    override suspend fun startManualRecording(request: RecordingRequest): Result<RecordingItem> = withContext(Dispatchers.IO) {
        runCatching {
            var storage = ensureStorageStateSync()
            // If the configured storage isn't writable (external drive removed, SAF permission
            // revoked, etc.), automatically fall back to internal app storage so recording
            // isn't silently blocked on every device.
            if (!storage.isWritable) {
                val (fallbackDir, fallbackBytes, fallbackWritable) = resolveStorageDetails(context, null)
                if (!fallbackWritable) {
    ...
    /**
     * Tee kaydi icin cikti dosyasini hazirlar.
     * 3 asamali garanti kayit dizini kullanir:
     * 1. Public Movies/KaynanamTV (Hayatta kalir, galeri dogrudan gorur)
     * 2. External App-Private Movies/KaynanamTV (Izin gerektirmez, 100% yazilir)
     * 3. Internal Cache/Files (Son care)
     * Kayit bitince galeriye taranir.
     */
    override suspend fun prepareTeeRecordingOutput(
        request: RecordingRequest
    ): Result<Pair<String, OutputStream>> = withContext(Dispatchers.IO) {
        runCatching {
            val fileName = sanitizeRecordingFileName(
                channelName = request.channelName,
                programTitle = request.programTitle,
                startMs = request.scheduledStartMs,
                pattern = "ChannelName_yyyy-MM-dd_HH-mm_ProgramTitle.mp4"
            )

            var outputStream: OutputStream? = null
            var finalFile: java.io.File? = null

            // 1. Asama: Public Movies Klasoru
            try {
                val publicMovies = android.os.Environment.getExternalStoragePublicDirectory(
                    android.os.Environment.DIRECTORY_MOVIES
                )
                val kaynanamDir = java.io.File(publicMovies, "KaynanamTV")
                kaynanamDir.mkdirs()
                val file = java.io.File(kaynanamDir, fileName)
                outputStream = java.io.FileOutputStream(file, false).buffered()
                finalFile = file
                Log.i(TAG, "tee-output public Movies path: ${file.absolutePath}")
            } catch (e: Exception) {
                Log.w(TAG, "Public Movies folder write failed, trying fallback: ${e.message}")
            }

            // 2. Asama: External App-Private Klasoru (Izin gerekmez)
            if (outputStream == null) {
                try {
                    val privateMovies = context.getExternalFilesDir(android.os.Environment.DIRECTORY_MOVIES)
                    val kaynanamDir = java.io.File(privateMovies, "KaynanamTV")
                    kaynanamDir.mkdirs()
                    val file = java.io.File(kaynanamDir, fileName)
                    outputStream = java.io.FileOutputStream(file, false).buffered()
                    finalFile = file
                    Log.i(TAG, "tee-output app-private path: ${file.absolutePath}")
                } catch (e: Exception) {
                    Log.w(TAG, "App-private folder write failed, trying internal: ${e.message}")
                }
            }

            // 3. Asama: Internal Storage (Son care)
            if (outputStream == null) {
                val internalDir = java.io.File(context.filesDir, "KaynanamTV")
                internalDir.mkdirs()
                val file = java.io.File(internalDir, fileName)
                outputStream = java.io.FileOutputStream(file, false).buffered()
                finalFile = file
                Log.i(TAG, "tee-output internal path: ${file.absolutePath}")
            }

            val fileToUse = finalFile ?: throw java.io.IOException("Hiçbir depolama alanına yazılamadı.")
            val streamToUse = outputStream ?: throw java.io.IOException("Yazma akışı oluşturulamadı.")

            // DB'ye kayit satiri olustur
            val scheduleId = recordingScheduleDao.insert(
                com.kaynanamtv.data.local.entity.RecordingScheduleEntity(
                    ...
                    recurrence = com.kaynanamtv.domain.model.RecordingRecurrence.NONE,
            ...
            val run = com.kaynanamtv.data.local.entity.RecordingRunEntity(
                ...
                outputUri = null,
                outputDisplayPath = fileToUse.absolutePath,
                startedAtMs = now,
                scheduleEnabled = false,
                priority = request.priority
            )
            recordingRunDao.insert(run)
            Log.i(TAG, "tee-output prepared runId=${run.id} path=${fileToUse.absolutePath}")
            run.id to streamToUse
        }.fold(
            onSuccess = { Result.success(it) },
            onFailure = { error -> Result.error(error.message ?: "Kayit hazirlanamadi.", error) }
        )
    }

    /**
     * Tee kaydi tamamlama: COMPLETED olarak isaretle, galeriye tara.
     */
    override suspend fun finalizeTeeRecording(
        recordingId: String,
        channelName: String,
        programTitle: String?
    ) = withContext(Dispatchers.IO) {
        val run = recordingRunDao.getById(recordingId) ?: return@withContext
        val now = System.currentTimeMillis()
        ...
        val path = run.outputDisplayPath
        if (!path.isNullOrBlank()) {
            addRecordingToMediaStore(
                ...
                durationMs = (now - (run.startedAtMs ?: now)).coerceAtLeast(0)
            )
        }
        Log.i(TAG, "tee-recording finalized runId=$recordingId path=$path")
    }
    ...
    private suspend fun completeRun(recordingId: String) {
        ...
        if (!path.isNullOrBlank()) {
            addRecordingToMediaStore(
                ...
                programTitle = run.programTitle,
                durationMs = if (run.endedAtMs != null && run.startedAtMs != null) {
                    (run.endedAtMs!! - run.startedAtMs!!).coerceAtLeast(0)
                } else null
            )
        }
    }
