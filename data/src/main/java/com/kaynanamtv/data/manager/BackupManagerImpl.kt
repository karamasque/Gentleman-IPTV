...
import com.kaynanamtv.data.local.dao.VirtualGroupDao
...
import com.kaynanamtv.data.preferences.ParentalPinBackupData
import com.kaynanamtv.data.preferences.PreferencesRepository
import com.kaynanamtv.data.security.CredentialCrypto
import com.kaynanamtv.domain.manager.BackupData
import com.kaynanamtv.domain.manager.BackupConflictStrategy
import com.kaynanamtv.domain.manager.BackupImportPlan
import com.kaynanamtv.domain.manager.BackupImportResult
import com.kaynanamtv.domain.manager.BackupManager
import com.kaynanamtv.domain.manager.BackupPreview
import com.kaynanamtv.domain.manager.RecordingScheduleImportDisposition
import com.kaynanamtv.domain.manager.RecordingScheduleImportOutcome
import com.kaynanamtv.domain.manager.RecordingScheduleImportSummary
import com.kaynanamtv.domain.manager.ProtectedCategoryBackup
import com.kaynanamtv.domain.manager.RecordingManager
import com.kaynanamtv.domain.manager.ScheduledRecordingBackup
...
class BackupManagerImpl @Inject constructor(
    ...
    override suspend fun exportConfig(uriString: String): com.kaynanamtv.domain.model.Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            } ?: return@withContext com.kaynanamtv.domain.model.Result.error("Failed to open output stream")

            com.kaynanamtv.domain.model.Result.success(Unit)
        } catch (e: Exception) {
            com.kaynanamtv.domain.model.Result.error("Failed to export backup: ${e.message}", e)
        }
    }
    ...
    ): com.kaynanamtv.domain.model.Result<BackupImportResult> = withContext(Dispatchers.IO) {
        try {
            val backupData = readBackupData(uriString)
                ?: return@withContext com.kaynanamtv.domain.model.Result.error("Failed to open input stream")

            if (backupData.version > CURRENT_BACKUP_VERSION) {
                return@withContext com.kaynanamtv.domain.model.Result.error("Unsupported backup version")
            }
            if (backupData.isStructurallyEmpty()) {
                return@withContext com.kaynanamtv.domain.model.Result.error("Backup file does not contain any importable data")
            }
            if (!verifyChecksum(backupData)) {
                return@withContext com.kaynanamtv.domain.model.Result.error("Backup file is corrupted (checksum mismatch)")
            }
            ...
            com.kaynanamtv.domain.model.Result.success(
                BackupImportResult(
            ...
            com.kaynanamtv.domain.model.Result.error("Failed to import backup: ${e.message}", e)
        }
    }
    ...
    private suspend fun restorePreferences(prefs: Map<String, String>) {
        ...
        prefs["appLandingDestination"]?.takeIf { it.isNotBlank() }?.let { savedDestination ->
            preferencesRepository.setAppLandingDestination(
                com.kaynanamtv.domain.model.AppLandingDestination.fromStorage(savedDestination)
            )
        }
        ...
        val legacyDecoderMode = prefs["playerDecoderMode"]
            ...
            ?.let { savedMode ->
                com.kaynanamtv.domain.model.DecoderMode.entries
        ...
        prefs["playerAudioOutputPreference"]?.takeIf { it.isNotBlank() }?.let { savedPreference ->
            val preference = com.kaynanamtv.domain.model.AudioOutputPreference.entries
        ...
        prefs["playerSurfaceMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val surfaceMode = com.kaynanamtv.domain.model.PlayerSurfaceMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (surfaceMode != null) {
        ...
        prefs["playerLiveStreamFormatMode"]?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val formatMode = com.kaynanamtv.domain.model.LiveStreamFormatMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
            if (formatMode != null) {
        ...
        (prefs["playerVodHttpProtocolMode"] ?: prefs["playerMovieHttpProtocolMode"])?.takeIf { it.isNotBlank() }?.let { savedMode ->
            val protocolMode = com.kaynanamtv.domain.model.VodHttpProtocolMode.entries
                .firstOrNull { entry -> entry.name == savedMode }
    ...
    private suspend fun restoreSavedLibrary(
        ...
        val categoriesByProviderId = mutableMapOf<Long, List<com.kaynanamtv.domain.model.Category>>()
    ...
    private fun resolveProviderIdMap(
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.kaynanamtv.domain.model.Provider>
    ...
    private suspend fun restorePlaybackHistory(
        history: List<com.kaynanamtv.domain.model.PlaybackHistory>,
        storedProviders: List<ProviderEntity>,
        backupProviders: List<com.kaynanamtv.domain.model.Provider>,
        conflictStrategy: BackupConflictStrategy
    ) {
...
internal fun com.kaynanamtv.domain.model.RecordingItem.toScheduledRecordingBackup(
    provider: com.kaynanamtv.domain.model.Provider,
...
internal suspend fun importScheduledRecordingBackups(
    ...
    storedProviders: List<ProviderEntity>,
    existingSchedules: MutableList<com.kaynanamtv.domain.model.RecordingItem>,
    conflictStrategy: BackupConflictStrategy,
    recordingManager: RecordingManager,
...
private fun com.kaynanamtv.domain.model.Provider.toSecureEntityForBackup(
...
private val PROVIDER_LIST_TYPE: Type = object : TypeToken<List<com.kaynanamtv.domain.model.Provider>>() {}.type
private val FAVORITE_LIST_TYPE: Type = object : TypeToken<List<com.kaynanamtv.domain.model.Favorite>>() {}.type
private val VIRTUAL_GROUP_LIST_TYPE: Type = object : TypeToken<List<com.kaynanamtv.domain.model.VirtualGroup>>() {}.type
private val PLAYBACK_HISTORY_LIST_TYPE: Type = object : TypeToken<List<com.kaynanamtv.domain.model.PlaybackHistory>>() {}.type
