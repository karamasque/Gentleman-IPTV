...
/** HTTP durum koduna göre uygun exception oluşturur. 403/401 auth hataları için daha açıklayıcı mesaj verir. */
private fun httpFailureException(code: Int): IOException = when (code) {
    403 -> UnsupportedRecordingException(
        "Sunucu bu yayının kaydedilmesine izin vermedi (HTTP 403). " +
        "Bu kanal kaydı desteklemiyor veya oturum süresi dolmuş olabilir.",
        RecordingFailureCategory.NETWORK
    )
    401 -> UnsupportedRecordingException(
        "Yayın kaydı için yetkilendirme başarısız oldu (HTTP 401). " +
        "Sağlayıcı bilgilerinizi kontrol edin.",
        RecordingFailureCategory.NETWORK
    )
    else -> IOException("Recording stream failed with HTTP $code")
}

private fun isTransientFailure(error: Throwable): Boolean {
    val msg = error.message.orEmpty().lowercase(Locale.ROOT)
    // 403/401 auth hataları kalıcıdır — retry etmeye gerek yok
    if (error is UnsupportedRecordingException) return false
