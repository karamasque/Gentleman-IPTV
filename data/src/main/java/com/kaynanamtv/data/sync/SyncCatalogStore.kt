...
import com.kaynanamtv.data.local.entity.CategoryImportStageEntity
import com.kaynanamtv.data.local.entity.ChannelEntity
import com.kaynanamtv.data.local.entity.ChannelImportStageEntity
import com.kaynanamtv.data.local.entity.MovieEntity
import com.kaynanamtv.data.local.entity.MovieImportStageEntity
import com.kaynanamtv.data.local.entity.SeriesEntity
import com.kaynanamtv.data.local.entity.SeriesImportStageEntity
import com.kaynanamtv.data.local.entity.TmdbIdentityEntity
...
internal class SyncCatalogStore(
    ...
    suspend fun replaceLiveCatalog(providerId: Long, categories: List<CategoryEntity>?, channels: List<ChannelEntity>): Int {
        ...
        try {
            ...
            val tStage = System.currentTimeMillis()
            insertStageRows(limitedChannels, catalogSyncDao::insertChannelStages)
            val tStageEnd = System.currentTimeMillis()
            Log.i("TTFC_BENCHMARK", "[DB_STAGE_INSERT] channels=${limitedChannels.size} batchSize=$STAGE_BATCH_SIZE dur=${tStageEnd - tStage}ms")
    ...
    private suspend fun applyCategories(
        ...
        val existingCategories = categoryDao.getByProviderAndTypeSync(providerId, type)
        if (existingCategories.isNotEmpty()) {
            val stagedByCategoryId = catalogSyncDao.getCategoryStages(providerId, sessionId, type)
                .associateBy { it.categoryId }
            val changed = existingCategories
                .mapNotNull { current ->
    ...
    private fun fingerprint(vararg values: String): String {
        ...
        for (value in values) {
            for (i in 0 until value.length) {
                hash = hash xor value[i].code.toLong()
                hash *= 1099511628211L
            }
            hash = hash xor 0L
            hash *= 1099511628211L
