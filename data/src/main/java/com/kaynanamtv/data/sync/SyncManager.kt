...
import com.kaynanamtv.data.local.entity.ChannelGuideSyncEntity
import com.kaynanamtv.data.local.entity.MovieEntity
import com.kaynanamtv.data.local.entity.SeriesEntity
import com.kaynanamtv.data.local.entity.XtreamContentIndexEntity
import com.kaynanamtv.data.local.entity.XtreamIndexJobEntity
import com.kaynanamtv.data.local.entity.XtreamLiveOnboardingStateEntity
...
import com.kaynanamtv.data.remote.http.buildAppRequestProfile
...
import com.kaynanamtv.data.remote.stalker.StalkerProviderProfile
...
import com.kaynanamtv.data.remote.xtream.OkHttpXtreamApiService
import com.kaynanamtv.data.remote.xtream.XtreamAuthenticationException
import com.kaynanamtv.data.remote.xtream.XtreamNetworkException
import com.kaynanamtv.data.remote.xtream.XtreamParsingException
import com.kaynanamtv.data.remote.xtream.XtreamRequestException
import com.kaynanamtv.data.remote.xtream.XtreamResponseTooLargeException
...
private const val STALKER_INDEX_CATEGORY_SLICE_SIZE = 500
...
class SyncManager @Inject constructor(
    ...
    private val m3uImporter = SyncManagerM3uImporter(
        ...
        retryTransient = { block -> retryTransient(maxAttempts = 5, initialDelayMs = 2000L, block = block) },
        progress = ::progress,
    ...
    private val activePrefetches = ConcurrentHashMap.newKeySet<Long>()
    ...
    suspend fun syncEpg(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return com.kaynanamtv.domain.model.Result.error("Provider $providerId not found")
        ...
        return if (providerEntity.type == ProviderType.STALKER_PORTAL) {
            ...
            withProviderLock(providerId) {
                val freshProviderEntity = providerDao.getById(providerId)
                    ?: return@withProviderLock com.kaynanamtv.domain.model.Result.error("Provider $providerId not found")
    ...
    private suspend fun syncEpgLocked(
        providerEntity: com.kaynanamtv.data.local.entity.ProviderEntity,
        force: Boolean,
        ...
        publishSyncState(providerId, SyncState.Syncing("Rehber (EPG) verileri indiriliyor..."))
        ...
        return try {
            ...
            updateXtreamEpgJobState(
                ...
                lastSuccessAt = finishedAt.takeIf { !epgResult.hasRetryableFailure },
            ...
            com.kaynanamtv.domain.model.Result.error(safeMessage, e)
        }
    }
    ...
    suspend fun rebuildXtreamIndex(
        ...
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.kaynanamtv.domain.model.Result.error("Index rebuild is only available for Xtream providers")
        }
        ...
        publishSyncState(providerId, SyncState.Syncing("Dizin yeniden yapılandırması hazırlanıyor..."))
        val now = System.currentTimeMillis()
        val warnings = mutableListOf<String>()

        try {
            withContext(Dispatchers.IO) {
                progress(providerId, onProgress, "Mevcut dizin kayıtları eski olarak işaretleniyor...")
    ...
    private suspend fun syncXtreamIndexFirst(
        ...
        if (provider.epgSyncMode == ProviderEpgSyncMode.UPFRONT) {
            updateXtreamEpgJobState(
                provider = provider,
                state = "RUNNING",
                now = now,
    ...
    private suspend fun syncXtreamCategoryShell(
        ...
        progress(provider.id, onProgress, "$label kategorileri yükleniyor...")
        upsertXtreamIndexJob(
            providerId = provider.id,
        ...
        val categories = when (categoriesResult) {
            is com.kaynanamtv.domain.model.Result.Success -> categoriesResult.data
            is com.kaynanamtv.domain.model.Result.Error -> {
                val message = categoriesResult.message.lowercase()
                val isNonFatalError = message.contains("unexpected end of stream") ||
                ...
                if (isNonFatalError) {
                    Log.w(TAG, "Non-fatal Xtream category shell load failure for $contentType (ignoring): ${categoriesResult.message}")
                    emptyList()
                } else {
                    throw IllegalStateException("Failed to load $label categories: ${categoriesResult.message}", categoriesResult.exception)
                }
            }
            is com.kaynanamtv.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
        ...
        val targetState = if (categories.isEmpty()) "SUCCESS" else "QUEUED"
        upsertXtreamIndexJob(
            providerId = provider.id,
            section = contentType.name,
            state = targetState,
    ...
    suspend fun processQueuedXtreamIndexJobs(
        ...
        val providerEntity = providerDao.getById(providerId)
            ?: return@lock com.kaynanamtv.domain.model.Result.error("Provider $providerId not found")
        if (providerEntity.type != ProviderType.XTREAM_CODES) {
            return@lock com.kaynanamtv.domain.model.Result.success(Unit)
        }
        ...
        try {
            ...
            sections.forEach { contentType ->
                if (StalkerTrafficCoordinator.shouldDeferCatalogFetch(providerId)) {
                    Log.d(TAG, "Deferring Xtream index work for provider $providerId due to active playback")
                    return@forEach
                }
            ...
            if (warnings.isNotEmpty()) {
                ...
                com.kaynanamtv.domain.model.Result.error(warnings.first(), exception)
            } else {
                com.kaynanamtv.domain.model.Result.success(Unit)
            }
        } finally {
    ...
    private fun shouldRunXtreamSummaryIndex(job: XtreamIndexJobEntity?): Boolean {
        ...
        if (job.state in setOf("QUEUED", "RUNNING", "STALE", "FAILED_RETRYABLE")) return true
    ...
    private suspend fun processStalkerSummaryIndexSection(
        ...
        data class StalkerCategoryFetchResult(
            ...
            val result: Result<com.kaynanamtv.data.remote.stalker.StalkerPagedResult<out Any>>
        )
        ...
        upsertXtreamIndexJob(
            ...
            lastAttemptAt = now,
            lastSuccessAt = finishedAt.takeIf { finalState == "SUCCESS" || finalState == "PARTIAL" },
    ...
    private suspend fun processStalkerWildcardIndexSection(
        ...
        while (pagesProcessed < maxPages) {
            ...
            progress(provider.id, onProgress, "${xtreamIndexSectionLabel(contentType)} kütüphaneye ekleniyor: Tüm sayfalar - Sayfa $nextPage")
            markStalkerAttemptStarted(
                providerId = provider.id,
                contentType = contentType,
            ...
            when (val result = fetchStalkerWildcardSummaryPageWithRecovery(api, contentType, wildcardCategory.categoryId, nextPage)) {
                is Result.Success -> {
                    ...
                    val anomaly = detectStalkerPageAnomaly(
                        hydration = currentHydration,
                        requestedPage = nextPage,
                        pagedResult = com.kaynanamtv.data.remote.stalker.StalkerPagedResult(
                            items = visibleItems,
    ...
    private suspend fun markStalkerAttemptStarted(
        ...
        when (contentType) {
            ContentType.MOVIE -> movieCategoryHydrationDao.upsert(
                com.kaynanamtv.data.local.entity.MovieCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
            ...
            ContentType.SERIES -> seriesCategoryHydrationDao.upsert(
                com.kaynanamtv.data.local.entity.SeriesCategoryHydrationEntity(
                    providerId = providerId,
                    categoryId = categoryId,
                    lastHydratedAt = hydration?.lastHydratedAt ?: 0L,
                    itemCount = hydration?.itemCount ?: 0,
    ...
    private suspend fun fetchStalkerSummaryPageWithRecovery(
        ...
        categoryId: Long,
        page: Int
    ): Result<com.kaynanamtv.data.remote.stalker.StalkerPagedResult<out Any>> {
    ...
    private suspend fun fetchStalkerSummaryFirstPageFallback(
        ...
        contentType: ContentType,
        categoryId: Long
    ): Result<com.kaynanamtv.data.remote.stalker.StalkerPagedResult<out Any>> = when (contentType) {
        ContentType.MOVIE -> when (val result = api.getVodStreams(categoryId)) {
            is Result.Success -> Result.success(
                com.kaynanamtv.data.remote.stalker.StalkerPagedResult(
                    items = result.data,
                    page = 1,
                    totalPages = 1,
    ...
    private fun detectStalkerPageAnomaly(
        hydration: StalkerHydrationSnapshot?,
        requestedPage: Int,
        pagedResult: com.kaynanamtv.data.remote.stalker.StalkerPagedResult<out Any>,
        pageFingerprint: String?
    ): String? {
    ...
    private suspend fun processXtreamSummaryIndexSection(
        ...
        for (workItem in workItems) {
            if (StalkerTrafficCoordinator.shouldDeferCatalogFetch(provider.id)) {
                Log.d(TAG, "Deferring Xtream summary index loop for provider ${provider.id} due to active playback")
                break
            }
            val category = workItem.category
            // Add a 150ms delay to prevent rate-limiting/banning by the IPTV provider's firewall
            kotlinx.coroutines.delay(150)
            progress(provider.id, onProgress, "${xtreamIndexSectionLabel(contentType)} kütüphaneye ekleniyor: ${category.name}")
        ...
        upsertXtreamIndexJob(
            ...
            lastAttemptAt = now,
            lastSuccessAt = if (finalState == "SUCCESS" || finalState == "PARTIAL") finishedAt else null,
            lastError = lastError?.let(::sanitizeThrowableMessage)
        )
    ...
    private suspend fun streamFullXtreamSummaryIndex(
        ...
        progress(provider.id, onProgress, "${xtreamIndexSectionLabel(contentType)} kütüphaneye ekleniyor...")
        upsertXtreamIndexJob(
            providerId = provider.id,
            section = contentType.name,
            state = "RUNNING",
    ...
    private fun xtreamIndexSectionLabel(contentType: ContentType): String = when (contentType) {
        ...
        ContentType.SERIES_EPISODE -> "Bölümler"
    }
    ...
    private suspend fun processXtreamLiveIndexBackfillSection(
        ...
        val now = System.currentTimeMillis()
        progress(providerId, onProgress, "Canlı TV listesi güncelleniyor...")
        upsertXtreamIndexJob(
            providerId = providerId,
            section = ContentType.LIVE.name,
            state = "RUNNING",
    ...
    private suspend fun syncM3u(
        ...
        if (!force && StalkerTrafficCoordinator.shouldDeferCatalogFetch(provider.id)) {
            Log.d(TAG, "Deferring M3U sync for provider ${provider.id} due to active playback")
            return SyncOutcome(partial = true, warnings = listOf("M3U sync deferred during active playback"))
        }
        val warnings = mutableListOf<String>()
        var metadata = syncMetadataRepository.getMetadata(provider.id) ?: SyncMetadata(provider.id)
        val now = System.currentTimeMillis()
        var activeProvider = provider
        val currentUrl = activeProvider.m3uUrl.ifBlank { activeProvider.serverUrl }
        val upgradedUrl = m3uImporter.upgradeXtreamM3uUrl(currentUrl)
        if (upgradedUrl != currentUrl) {
            providerDao.updateM3uUrl(activeProvider.id, upgradedUrl)
            activeProvider = activeProvider.copy(m3uUrl = upgradedUrl, serverUrl = upgradedUrl)
        }

        if (force || ContentCachePolicy.shouldRefresh(metadata.lastLiveSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            val stats = withContext(Dispatchers.IO) { m3uImporter.importPlaylist(activeProvider, onProgress) }
            ...
            if (activeProvider.epgUrl.isBlank()) {
                val derivedEpgUrl = stats.header.tvgUrl?.takeIf { UrlSecurityPolicy.validateOptionalEpgUrl(it) == null }
                    ?: m3uImporter.deriveXtreamEpgUrl(activeProvider.m3uUrl.ifBlank { activeProvider.serverUrl })
                if (!derivedEpgUrl.isNullOrBlank()) {
                    providerDao.updateEpgUrl(activeProvider.id, derivedEpgUrl)
                    activeProvider = activeProvider.copy(epgUrl = derivedEpgUrl)
                }
            }
            metadata = metadata.copy(
                lastLiveSync = now,
        ...
        if (activeProvider.epgUrl.isNotBlank() && activeProvider.epgSyncMode != ProviderEpgSyncMode.SKIP) {
            updateXtreamEpgJobState(
            ...
            updateXtreamEpgJobState(
                ...
                now = finishedAt,
                indexedRows = programDao.countByProvider(activeProvider.id),
                lastAttemptAt = now,
                lastSuccessAt = finishedAt.takeIf { !epgResult.hasRetryableFailure },
                lastError = epgResult.warnings.takeIf { it.isNotEmpty() }?.joinToString("; ")
            )
            warnings += epgResult.warnings
        } else if (activeProvider.epgSyncMode != ProviderEpgSyncMode.SKIP) {
            upsertXtreamIndexJob(
            ...
            scheduleBackgroundEpgSync(activeProvider.id)
        }
    ...
    private suspend fun syncStalker(
        ...
        progress(provider.id, onProgress, "Portale bağlanılıyor...")
        val api = createStalkerSyncProvider(provider)
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastMovieSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            progress(provider.id, onProgress, "Filmler hazırlanıyor...")
        ...
        if (force || ContentCachePolicy.shouldRefresh(metadata.lastSeriesSuccess, ContentCachePolicy.CATALOG_TTL_MILLIS, now)) {
            progress(provider.id, onProgress, "Diziler hazırlanıyor...")
    ...
    private suspend fun syncJellyfin(
        ...
        try {
            ...
            progress(provider.id, onProgress, "Jellyfin kütüphanesi yükleniyor...")
            ...
            if (resolvedMovies is com.kaynanamtv.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Movies error: ${resolvedMovies.message}")
            if (resolvedSeries is com.kaynanamtv.domain.model.Result.Error) android.util.Log.e("JellyfinSync", "Series error: ${resolvedSeries.message}")

            if (resolvedMovies is com.kaynanamtv.domain.model.Result.Error && resolvedSeries is com.kaynanamtv.domain.model.Result.Error) {
            ...
            val movieEntities = (resolvedMovies as? com.kaynanamtv.domain.model.Result.Success)?.data.orEmpty()
            val seriesEntities = (resolvedSeries as? com.kaynanamtv.domain.model.Result.Success)?.data.orEmpty()

            if (movieEntities.isNotEmpty()) {
                progress(provider.id, onProgress, "Jellyfin filmleri içe aktarılıyor...")
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.kaynanamtv.domain.model.ContentType.MOVIE.name, listOf(
                        com.kaynanamtv.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 1L, name = "Filmler",
            ...
            if (seriesEntities.isNotEmpty()) {
                progress(provider.id, onProgress, "Jellyfin dizileri içe aktarılıyor...")
                ...
                transactionRunner.inTransaction {
                    categoryDao.replaceAll(provider.id, com.kaynanamtv.domain.model.ContentType.SERIES.name, listOf(
                        com.kaynanamtv.data.local.entity.CategoryEntity(
                            providerId = provider.id, categoryId = 2L, name = "Diziler",
                            type = com.kaynanamtv.domain.model.ContentType.SERIES
                        )
                    ))
                }
            }
    ...
    private suspend fun syncProviderEpg(
        ...
        when (provider.type) {
            ProviderType.XTREAM_CODES -> {
                if (StalkerTrafficCoordinator.shouldDeferCatalogFetch(provider.id)) {
                    Log.d(TAG, "Deferring EPG sync for provider ${provider.id} due to active playback")
                    return EpgSyncResult(warnings = listOf("EPG sync deferred during active playback"), hasRetryableFailure = false)
                }
                if (shouldUseProviderGuide(guidePolicy) &&
                    ...
                    try {
                        progress(provider.id, onProgress, "Rehber (EPG) verileri indiriliyor...")
        ...
        if (shouldUseExternalGuide(guidePolicy)) {
            try {
                progress(provider.id, onProgress, "Harici EPG kaynakları güncelleniyor...")
                retryTransient {
                    requireResult(
                ...
                progress(provider.id, onProgress, "EPG eşleşmeleri çözümleniyor...")
            ...
            progress(provider.id, onProgress, "Bu sağlayıcı için rehber verisi devre dışı...")
            epgSourceRepository.resolveForProvider(provider.id, hiddenLiveCategoryIds)
        } else {
            progress(provider.id, onProgress, "Sağlayıcı EPG eşleşmeleri çözümleniyor...")
            epgSourceRepository.resolveForProvider(provider.id, hiddenLiveCategoryIds)
        }
    ...
    private suspend fun syncEpgOnly(
        ...
        progress(provider.id, onProgress, "EPG yeniden deneniyor...")
        val hiddenLiveCategoryIds = preferencesRepository.getHiddenCategoryIds(provider.id, ContentType.LIVE).first()
        val guidePolicy = provider.guideSourcePolicy
    ...
    private suspend fun syncStalkerPortalEpg(
        ...
        val insertBuffer = ArrayList<com.kaynanamtv.data.local.entity.ProgramEntity>(STALKER_GUIDE_PROGRAM_BATCH_SIZE)
        ...
        runCatching {
            ...
            api.streamBulkEpg(periodHours = 6) { program ->
                ...
                if (result is com.kaynanamtv.domain.model.Result.Error) {
        ...
        fallbackGuideRequests.forEachIndexed { index, request ->
            ...
            progress(provider.id, onProgress, "Portal rehber verileri indiriliyor... ${index + 1} / ${fallbackGuideRequests.size}")
            runCatching {
                ...
                if (streamResult is com.kaynanamtv.domain.model.Result.Error) {
    ...
    private suspend fun syncLiveOnly(
        ...
        when (provider.type) {
            ProviderType.XTREAM_CODES -> {
                progress(provider.id, onProgress, "Canlı TV yeniden deneniyor...")
                val useTextClassification = preferencesRepository.useXtreamTextClassification.first()
                val enableBase64TextCompatibility = preferencesRepository.xtreamBase64TextCompatibility.first()
                val hiddenLiveCategoryIds = preferencesRepository.getHiddenCategoryIds(provider.id, ContentType.LIVE).first()
                val api = createXtreamSyncProvider(provider, useTextClassification, enableBase64TextCompatibility)
    ...
    private suspend fun syncMoviesOnly(
        ...
        when (provider.type) {
            ProviderType.XTREAM_CODES -> {
                progress(provider.id, onProgress, "Filmler kütüphaneye ekleniyor...")
                val useTextClassification = preferencesRepository.useXtreamTextClassification.first()
                val enableBase64TextCompatibility = preferencesRepository.xtreamBase64TextCompatibility.first()
                val api = createXtreamSyncProvider(provider, useTextClassification, enableBase64TextCompatibility)
                processXtreamSummaryIndexSection(
            ...
            ProviderType.M3U -> {
                progress(provider.id, onProgress, "Filmler yeniden deneniyor...")
                val stats = withContext(Dispatchers.IO) {
                    m3uImporter.importPlaylist(provider, onProgress, includeLive = true, includeMovies = true)
                }
    ...
    private suspend fun syncSeriesOnly(
        ...
        when (provider.type) {
            ProviderType.XTREAM_CODES -> {
                progress(provider.id, onProgress, "Diziler kütüphaneye ekleniyor...")
                val useTextClassification = preferencesRepository.useXtreamTextClassification.first()
                val enableBase64TextCompatibility = preferencesRepository.xtreamBase64TextCompatibility.first()
                val api = createXtreamSyncProvider(provider, useTextClassification, enableBase64TextCompatibility)
                processXtreamSummaryIndexSection(
    ...
    private fun publishSyncState(providerId: Long, state: SyncState) {
        ...
        if ((state is SyncState.Success || state is SyncState.Partial) && activePrefetches.add(providerId)) {
            prefetchProviderMedia(providerId, state)
        }
    }

    private fun prefetchProviderMedia(providerId: Long, finalState: SyncState) {
        val job = kotlinx.coroutines.SupervisorJob()
        val scope = kotlinx.coroutines.CoroutineScope(Dispatchers.IO + job)
        scope.launch {
            try {
                // NOT: Burada publishSyncState çağrılmıyor — UI state'ini
                // "Syncing" durumuna geri döndürmemek için syncStateTracker
                // doğrudan değiştirilmiyor. Prefetch tamamen sessiz çalışır;
                // kullanıcı TV izlemeye devam edebilir.
                val channels = channelDao.getByProvider(providerId).first()
                val movies = movieDao.getByProvider(providerId).first()
                val series = seriesDao.getByProvider(providerId).first()

                val urls = mutableSetOf<String>()
                channels.forEach { if (!it.logoUrl.isNullOrBlank()) urls.add(it.logoUrl) }
                movies.forEach { if (!it.posterUrl.isNullOrBlank()) urls.add(it.posterUrl) }
                series.forEach { if (!it.posterUrl.isNullOrBlank()) urls.add(it.posterUrl) }

                if (urls.isNotEmpty()) {
                    mediaPrefetcher.prefetchMediaImages(urls.toList())
                }
            } catch (e: Exception) {
                Log.e(TAG, "Görsel önbellekleme hatası (Provider $providerId)", e)
            } finally {
                activePrefetches.remove(providerId)
                // Prefetch bitti: asıl son durumu (Success/Partial) yayınla.
                // publishSyncState yerine doğrudan tracker'a yazıyoruz —
                // döngüye girmemek ve aktivePrefetches'e tekrar eklememek için.
                syncStateTracker.publish(providerId, finalState)
                job.cancel()
            }
        }
    }
    ...
    private suspend fun loadStalkerChannelsByCategory(
        api: StalkerProvider,
        categories: List<com.kaynanamtv.domain.model.Category>,
        onProgress: ((String) -> Unit)?
    ...
    private suspend fun fetchStalkerLiveChannelsByCategory(
        ...
        if (concurrency > 1 && catalogStrategySupport.shouldRetryFailedCategories(
            ...
            progress(provider.id, onProgress, "Başarısız Canlı TV kategorileri yeniden deneniyor: 0/$failedRetryTotal...")
            timedOutcomes = continueFailedCategoryOutcomes(
                ...
                onCategoryRetried = { completed, total, _ ->
                    progress(provider.id, onProgress, "Başarısız Canlı TV kategorileri yeniden deneniyor: $completed/$total...")
                }
            )
            warnings = (
    ...
    private suspend fun stalkerCatalogAccessDiagnostic(
        ...
        val profile = when (val profileResult = api.getAccountProfile()) {
            is com.kaynanamtv.domain.model.Result.Success -> profileResult.data
            else -> return null
        }
    ...
    private fun <T> requireResult(result: com.kaynanamtv.domain.model.Result<T>, fallbackMessage: String): T {
        return when (result) {
            is com.kaynanamtv.domain.model.Result.Success -> result.data
            is com.kaynanamtv.domain.model.Result.Error -> throw IllegalStateException(result.message.ifBlank { fallbackMessage }, result.exception)
            is com.kaynanamtv.domain.model.Result.Loading -> throw IllegalStateException("Unexpected loading state")
        }
    }
