package com.kaynanamtv.data.parser

import com.kaynanamtv.domain.model.Channel
import com.kaynanamtv.domain.model.Movie
import com.kaynanamtv.domain.util.ChannelNormalizer
import com.kaynanamtv.domain.util.StreamEntryUrlPolicy
...
class M3uParser {
    ...
    fun parse(inputStream: InputStream, playlistUrl: String? = null): ParseResult {
            ...
            reader.lineSequence()
                ...
                .forEach { line ->
                    when {
                        ...
                        pendingExtinf != null -> {
                            parseEntry(pendingExtinf!!, line, header.userAgent, playlistUrl)?.let(entries::add)
                            pendingExtinf = null
                        }
                        else -> {
        ...
        reader.use {
            for (rawLine in reader.lineSequence()) {
                ...
                when {
                    ...
                    pendingExtinf != null -> {
                        parseEntry(pendingExtinf!!, line, header.userAgent, playlistUrl)?.let { onEntry(it) }
                        pendingExtinf = null
                    }
                    else -> {
                        // Non-comment, non-URL line with no pending EXTINF — skip
    ...
    fun parseToChannels(inputStream: InputStream, providerId: Long, playlistUrl: String? = null): List<Channel> {
        return parse(inputStream, playlistUrl).entries.mapIndexed { index, entry ->
            Channel(
    ...
    private fun parseEntry(extinf: ParsedExtinf, url: String, globalUserAgent: String?, playlistUrl: String?): M3uEntry? {
        if (url.isBlank() || extinf.name.isBlank()) return null
        val normalizedUrl = normalizeUrl(url, playlistUrl)
        if (!isAllowedStreamUrl(normalizedUrl)) return null
        return M3uEntry(
    ...
    private fun normalizeUrl(url: String, playlistUrl: String?): String {
        val trimmed = url.trim()
        if (trimmed.startsWith("/") || trimmed.startsWith("\\")) {
            return "file://" + trimmed.replace('\\', '/')
        }
        val isWindowsDrivePath = trimmed.length >= 3 && trimmed[1] == ':' && (trimmed[2] == '\\' || trimmed[2] == '/')
        if (isWindowsDrivePath) {
            return "file:///" + trimmed.replace('\\', '/')
        }
        if (!trimmed.contains("://") && !playlistUrl.isNullOrBlank()) {
            if (playlistUrl.startsWith("file://")) {
                val parentDir = playlistUrl.substringBeforeLast('/')
    ...
    private fun parseExtinf(line: String): ParsedExtinf? {
        ...
        val metadata = if (commaIndex < 0) payload else payload.substring(0, commaIndex).trim()
        val rawName = if (commaIndex < 0) "" else payload.substring(commaIndex + 1).trim()
        ...
        val name = if (rawName.isEmpty()) {
            attributes["tvg-name"]?.takeIf { it.isNotBlank() }
                ?: attributes["tvg-id"]?.takeIf { it.isNotBlank() }
                ?: "Unnamed Channel"
        } else {
            rawName
        }
    ...
    companion object {
        ...
        fun isVodEntry(entry: M3uEntry): Boolean {
            val url = entry.url.lowercase(java.util.Locale.ROOT)
            val rawGroup = entry.groupTitle.lowercase(java.util.Locale.ROOT)
            val group = rawGroup
                .replace('ı', 'i')
                .replace('ş', 's')
                .replace('ğ', 'g')
            ...
            if (url.contains("type=live")) return false
            if (url.contains("/movie/") || url.contains("/movies/")) return true
            if (url.contains("/series/")) return true

            if (url.endsWith(".mp4") || url.endsWith(".mkv") ||
                url.endsWith(".avi") || url.endsWith(".mp3")) return true

            val isLiveGroup = group.contains("live") ||
                group.contains("tv") ||
                group.contains("kanal") ||
                group.contains("channel") ||
                group.contains("sport") ||
                group.contains("spor") ||
                group.contains("haber") ||
                group.contains("news") ||
                group.contains("yayin")

            if (isLiveGroup) return false

            val isVodGroup = group.contains("movie") ||
                group.contains("vod") ||
                group.contains("film") ||
