...
class XmltvParser {
    ...
    private fun newPullParser(inputStream: InputStream): XmlPullParser {
        ...
        runCatching {
            ...
            logger.log(Level.FINE, "XML pull parser does not support disabling process-docdecl", it)
        }
    ...
    suspend fun parseStreaming(
        ...
        try {
            while (eventType != XmlPullParser.END_DOCUMENT) {
                when (eventType) {
                    // Skip DOCTYPE declarations — providers commonly declare
                    // <!DOCTYPE tv SYSTEM "xmltv.dtd"> which triggers network
                    // resolution attempts when process-docdecl is not fully disabled.
                    XmlPullParser.DOCDECL -> { /* intentionally ignored */ }
                    XmlPullParser.START_TAG -> {
                        when (parser.name) {
                            "programme" -> {
                                inProgramme = true
                ...
                // Wrap parser.next() so a DTD resolution IOException (e.g. trying to fetch
                // "xmltv.dtd" over the network) is caught and converted to a DOCDECL skip
                // rather than aborting the entire parse with 0 programs.
                eventType = try {
                    parser.next()
                } catch (e: org.xmlpull.v1.XmlPullParserException) {
                    if (e.message?.contains("DOCTYPE", ignoreCase = true) == true ||
                        e.message?.contains("doctype", ignoreCase = true) == true ||
                        e.message?.contains("dtd", ignoreCase = true) == true) {
                        logger.log(Level.FINE, "Skipping DOCTYPE/DTD in XMLTV: ${e.message}")
                        XmlPullParser.COMMENT // treat as ignorable event and continue
                    } else throw e
                }
            }
        } catch (e: Exception) {
