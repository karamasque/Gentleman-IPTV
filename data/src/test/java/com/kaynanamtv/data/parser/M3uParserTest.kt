...
class M3uParserTest {
    ...
    @Test
    fun `isVodEntry_turkishLiveMovieCategory_returnsFalse`() {
        val entries = parseEntries(buildPlaylist(
            "http://live.example.com/movie_ch.ts" to "TR | SİNEMA - FİLM",
            "http://live.example.com/sinema.m3u8" to "FİLM BOX"
        ))
    ...
    @Test
    fun `parse_relativeAndAbsoluteLocalPaths_normalizesCorrectly`() {
        val m3u = """
            #EXTM3U
            #EXTINF:-1,Relative Video
            movies/video1.mp4
            #EXTINF:-1,Absolute Linux Path
            /storage/emulated/0/Movies/video2.mp4
            #EXTINF:-1,Absolute Windows Path
            C:\Movies\video3.mp4
        """.trimIndent()

        val result = parser.parse(m3u.byteInputStream(), playlistUrl = "file:///storage/emulated/0/Download/playlist.m3u")
        val entries = result.entries

        assertThat(entries).hasSize(3)
        assertThat(entries[0].url).isEqualTo("file:///storage/emulated/0/Download/movies/video1.mp4")
        assertThat(entries[1].url).isEqualTo("file:///storage/emulated/0/Movies/video2.mp4")
        assertThat(entries[2].url).isEqualTo("file:///C:/Movies/video3.mp4")
    }

    @Test
    fun `parse_emptyNameAndMissingComma_fallsBackCorrectly`() {
        val m3u = """
            #EXTM3U
            #EXTINF:-1 tvg-name="Fallback Name 1" tvg-id="id1",
            http://stream.example.com/1.ts
            #EXTINF:-1 tvg-name="Fallback Name 2" tvg-id="id2"
        ...
        assertThat(entries).hasSize(2)
        assertThat(entries[0].name).isEqualTo("Fallback Name 1")
        assertThat(entries[1].name).isEqualTo("Fallback Name 2")
    }
