...
/**
 * Automated Failure Regression Tests simulating various Xtream API & Storage failure states.
 * Ensures the app handles errors gracefully without crashing or sticking in an endless loading state.
 */
class FailureRegressionTest {

    private class ConfigurableFakeBackend(
        var categoriesStatusCode: Int = 200,
        var categoriesBody: String = """[{"category_id":"1","category_name":"Sports"}]""",
        var streamsStatusCode: Int = 200,
        var streamsBody: String = """[{"stream_id":101,"name":"Test Sports HD","category_id":"1"}]"""
    ) {
        fun okHttpClient(): OkHttpClient {
            return OkHttpClient.Builder()
                .addInterceptor { chain ->
                    ...
                    val (code, body) = when (action) {
                        "get_live_categories" -> categoriesStatusCode to categoriesBody
                        "get_live_streams" -> streamsStatusCode to streamsBody
                        else -> 200 to "[]"
                    }
                    Response.Builder()
                        .request(request)
    ...
    fun setup() {
        ...
        runBlocking {
            ...
            whenever(epgSourceRepo.resolveForProvider(any(), any())).thenReturn(com.kaynanamtv.domain.model.EpgResolutionSummary())
    ...
    private fun createSyncManager(backend: ConfigurableFakeBackend): SyncManager {
        return SyncManager(
            applicationContext = applicationContext,
            providerDao = FakeProviderDao(sampleProvider()),
    ...
    @Test
    fun `streams API 500 server error returns failure result without crashing`() = runTest {
        val backend = ConfigurableFakeBackend(
            categoriesStatusCode = 500,
            categoriesBody = """{"error":"Server error"}""",
            streamsStatusCode = 500,
            streamsBody = """{"error":"Server error"}"""
        )
        val manager = createSyncManager(backend)
    ...
    @Test
    fun `empty streams response handled gracefully without infinite loading`() = runTest {
        val backend = ConfigurableFakeBackend(streamsBody = "[]")
        val manager = createSyncManager(backend)
    ...
    @Test
    fun `malformed streams JSON does not crash app and completes gracefully`() = runTest {
        val backend = ConfigurableFakeBackend(streamsBody = "{ invalid json content }")
        val manager = createSyncManager(backend)
