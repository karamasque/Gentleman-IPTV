package com.kaynanamtv.player.ui
...
import com.kaynanamtv.player.R
...
class PlayerViewBinder(
    ...
    fun attachPlayer(player: androidx.media3.exoplayer.ExoPlayer?) {
        boundPlayerView?.let { pv ->
            if (pv.player !== player) {
                pv.player = player
            }
            pv.applyResizeMode(boundResizeMode)
            subtitleStyleController.apply(pv)
            applyInjectedCues(pv)
        }
    }
