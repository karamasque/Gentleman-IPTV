...
class PlaybackCodecSelectorTest {
    ...
    fun `software codec names are recognized`() {
        ...
        assertThat(PlaybackCodecSelector.isSoftwareCodec("c2.android.sw.hevc.decoder")).isTrue()
        assertThat(PlaybackCodecSelector.isSoftwareCodec("OMX.ffmpeg.avc.decoder")).isTrue()
    }

    @Test
    fun `vendor and hardware codec names are not software`() {
        assertThat(PlaybackCodecSelector.isSoftwareCodec("c2.android.hevc.decoder")).isFalse()
    ...
    private fun compatibilityRecord(
        ...
        successAt: Long,
        failedAt: Long
    ) = com.kaynanamtv.domain.model.PlaybackCompatibilityRecord(
        key = com.kaynanamtv.domain.model.PlaybackCompatibilityKey(
