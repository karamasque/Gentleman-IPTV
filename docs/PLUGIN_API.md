# KaynanamTV Plugin API

KaynanamTV plugins are companion Android APKs. They do not inject code into the
main application. Instead, KaynanamTV discovers installed APKs that expose a
bound service with the action `com.kaynanamtv.plugin.API` and talks to them
...
KaynanamTV from a declarative schema, or opened as a native plugin Activity when
...
```xml
<action android:name="com.kaynanamtv.plugin.API" />
```
...
- Any manual Android package install. KaynanamTV detects it after refreshing the
  Plugins screen.

## Manifest

KaynanamTV first asks the plugin service for its manifest. As a fallback, it
reads `com.kaynanamtv.plugin.MANIFEST_JSON` service metadata, then individual
...
```json
{
  ...
  "versionName": "1.0.0",
  "versionCode": 1,
  "description": "Adds external capabilities to KaynanamTV.",
...
```xml
<meta-data android:name="com.kaynanamtv.plugin.ID" android:value="com.example.plugin" />
<meta-data android:name="com.kaynanamtv.plugin.NAME" android:value="Example Plugin" />
<meta-data android:name="com.kaynanamtv.plugin.VERSION_NAME" android:value="1.0.0" />
<meta-data android:name="com.kaynanamtv.plugin.VERSION_CODE" android:value="1" />
<meta-data android:name="com.kaynanamtv.plugin.DESCRIPTION" android:value="Adds external capabilities to KaynanamTV." />
<meta-data android:name="com.kaynanamtv.plugin.PROVIDER_NAME" android:value="Example Provider" />
<meta-data android:name="com.kaynanamtv.plugin.CONFIGURATION_MODE" android:value="host.schema" />
<meta-data android:name="com.kaynanamtv.plugin.CONFIGURATION_ACTIVITY_ACTION" android:value="" />
<meta-data android:name="com.kaynanamtv.plugin.CAPABILITIES" android:value="provider.m3u,playback.prepare,cast.rewriteUrl,configuration.schema" />
```
...
- `provider.m3u`: the plugin can expose an M3U URL that KaynanamTV imports as a
...
- `configuration.schema`: the plugin exposes a declarative configuration schema
  that KaynanamTV renders with its own UI.
- `configuration.activity`: the plugin exposes a native Android Activity that
  KaynanamTV opens for configuration.
...
`handled=false` when the URL is not theirs. KaynanamTV then continues with other
enabled plugins or the original URL.

`MSG_REWRITE_CAST_URL` is Cast-specific. KaynanamTV sends the stream context that
...
`MSG_PREPARE_PLAYBACK` may also enrich the stream that KaynanamTV sends to the
player. When `handled=true` and `success=true`, the plugin can return:

- `output_url`: final playable URL. If omitted, KaynanamTV keeps `input_url`.
...
For `SMOOTH_STREAMING` + `clearkey`, KaynanamTV normalizes the ISML
...
simple actions. KaynanamTV owns the visual shell, validation placement, focus
...
`configurationMode` is `activity`, KaynanamTV treats the Activity as the active
...
KaynanamTV owns the visual implementation.
...
```json
{
  ...
  "description": "Settings rendered by KaynanamTV.",
  "sections": [
    {
      "id": "connection",
      "title": "Connection",
...
- `info`: read-only text rendered by KaynanamTV.
...
For `MSG_SET_CONFIGURATION_VALUES`, KaynanamTV sends the same JSON object in
...
For `MSG_RUN_CONFIGURATION_ACTION`, KaynanamTV sends `configuration_action_id`.
...
KaynanamTV starts `configurationActivityAction` with the plugin package set
...
Native plugin Activities should feel at home when launched from KaynanamTV and
...
- Design TV-first: every important control must be D-pad focusable and readable
  from couch distance.
- Keep the style restrained and close to KaynanamTV: dark background, compact
...
- Test both entry points: launched directly from the plugin APK and launched from
  KaynanamTV's Plugins screen.
- Test on the same device classes KaynanamTV supports, including TV devices such
...
- Expose exactly one `com.kaynanamtv.plugin.API` service.
