...
KaynanamTV can sync its backup file to a private folder of your Google Drive
...
your choice (e.g. `kaynanamtv-dev-<your-handle>`). Select it.
...
- **User type** → External
- **App name** → `KaynanamTV` (or your fork name)
...
| Field | Value |
|---|---|
| Name | `KaynanamTV — debug (<your handle>)` |
| Package name | `com.kaynanamtv.app` |
...
- **`kaynanamtv_credentials.json`** — `[{serverUrl, username, password}]`,
  cleartext.
...
Pre-M3 backups (no `kaynanamtv_credentials.json` in `appDataFolder`)
